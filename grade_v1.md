# Assignment 2 Grading: vlahosevan

**Final Score: 98/100 (A+)**

## Summary
- Database Models: 18/20
...
❌ **DB-03**: Task-Category relationship (task belongs to category)
   - Score: 4/6
   - Task includes category reference
   - Deduction: Category not nested as object in task response (-2 pts, minor)
...
✅ **DOCK-01**: docker-compose.yml defines all 4 services (app, db, redis, worker)
   - Score: 5/5
   - All 4 services defined: ['app', 'worker', 'db', 'redis']
...
- Task-Category relationship (task belongs to category): Category not nested as object in task response
